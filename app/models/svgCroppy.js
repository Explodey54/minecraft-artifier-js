function SvgCroppy() {
    ...
    switch (key) {
      case "width":
        if (value <= store.posData.selectRect.minWidth) {
          ...
          value >=
          store.posData.boundingRect.width - store.posData.selectRect.offsetX
        ) {
          store.posData.selectRect.width =
            store.posData.boundingRect.width - store.posData.selectRect.offsetX;
        } else {
      ...
      case "height":
        if (value <= store.posData.selectRect.minHeight) {
          ...
          value >=
          store.posData.boundingRect.height - store.posData.selectRect.offsetY
        ) {
          store.posData.selectRect.height =
            store.posData.boundingRect.height -
            store.posData.selectRect.offsetY;
        } else {
      ...
      case "offsetX":
        if (value <= 0) {
          ...
          value >=
          store.posData.boundingRect.width - store.posData.selectRect.width
        ) {
          store.posData.selectRect.offsetX =
            store.posData.boundingRect.width - store.posData.selectRect.width;
        } else {
      ...
      case "offsetY":
        if (value <= 0) {
          ...
          value >=
          store.posData.boundingRect.height - store.posData.selectRect.height
        ) {
          store.posData.selectRect.offsetY =
            store.posData.boundingRect.height - store.posData.selectRect.height;
        } else {
  ...
  this._setEventListeners = () => {
    ...
    for (let key in svgControls) {
      svgControls[key].addEventListener("mousedown", (e) => {
        ...
        grabbed.el =
          svgNodeStyleAliases[e.target.classList[1]] ||
          svgNodeStyleAliases[e.target.classList[0]];
        if (grabbed.el === undefined) {
          throw new Error(
            "[SvgCroppy] Selected node is not defined in aliases"
          );
        }
        ...
        grabbed.oldEndX =
          store.posData.selectRect.offsetX + store.posData.selectRect.width;
        grabbed.oldEndY =
          store.posData.selectRect.offsetY + store.posData.selectRect.height;
      });
    }
    ...
    document.addEventListener("mousemove", (e) => {
      if (e.target.constructor.name !== "HTMLHtmlElement") {
        if (
          e.target.parentNode.getAttribute("class") !== "croppy-svg" &&
          !grabbed.el
        ) {
          //hardcode
          return;
      ...
      if (grabbed.el) {
        ...
        switch (grabbed.el) {
          case "CORNER_LEFT_TOP":
            if (
              grabbed.oldOffsetX + startLocal.x > 0 &&
              grabbed.oldEndX >=
                grabbed.oldOffsetX +
                  startLocal.x +
                  store.posData.selectRect.minWidth
            ) {
              this._setPosDataSelectRect(
                "offsetX",
                grabbed.oldOffsetX + startLocal.x
              );
              this._setPosDataSelectRect(
                "width",
                grabbed.oldWidth - startLocal.x
              );
              ...
              this._setPosDataSelectRect(
                "offsetX",
                grabbed.oldEndX - store.posData.selectRect.minWidth
              );
              this._setPosDataSelectRect(
                "width",
                store.posData.selectRect.minWidth
            ...
            if (grabbed.oldOffsetY + startLocal.y <= 0) {
              ...
              grabbed.oldOffsetY +
                startLocal.y +
                store.posData.selectRect.minHeight
            ) {
              this._setPosDataSelectRect(
                "offsetY",
                grabbed.oldOffsetY + startLocal.y
              );
              this._setPosDataSelectRect(
                "height",
                grabbed.oldHeight - startLocal.y
              );
            } else {
              this._setPosDataSelectRect(
                "offsetY",
                grabbed.oldEndY - store.posData.selectRect.minHeight
              );
              this._setPosDataSelectRect(
                "height",
                store.posData.selectRect.minHeight
  ...
  this.render = () => {
    ...
    setAttr($cornerLeftTop, {
      x:
        store.posData.selectRect.offsetX -
        Math.round(store.style.cornerWidth / 2),
      y:
        store.posData.selectRect.offsetY -
        Math.round(store.style.cornerHeight / 2),
    });
    ...
    setAttr($barTop, {
      x:
        store.posData.selectRect.offsetX -
        Math.round(store.style.cornerWidth / 2) +
        store.style.cornerWidth,
      y:
        store.posData.selectRect.offsetY -
        Math.round(store.style.cornerHeight / 2),
      width:
        store.posData.selectRect.width - store.style.cornerHeight >= 0
          ? store.posData.selectRect.width - store.style.cornerHeight
          : 0,
    ...
    setAttr($barLeft, {
      ...
      y:
        store.posData.selectRect.offsetY -
        Math.round(store.style.cornerHeight / 2) +
        store.style.cornerHeight,
      width: store.style.cornerWidth,
      height:
        store.posData.selectRect.height - store.style.cornerHeight >= 0
          ? store.posData.selectRect.height - store.style.cornerHeight
          : 0,
    });
  ...
  this.init = (rootSelector) => {
    ...
    if ($root === null) {
      throw new Error("[SvgCroppy] rootNode is null");
    }
    ...
    const $svgBackgroundRect = this._storeSetSvgNode(
      "backgroundRect",
      $svg.querySelector(".croppy-svg-background")
    );
    const $svgSelectRect = this._storeSetSvgNode(
      "selectRect",
      $svg.querySelector(".croppy-svg-select")
    );

    this._storeSetSvgNode(
      "selectRectMask",
      $svg.querySelector(".croppy-svg-mask")
    );

    this._storeSetSvgNode(
      "cornerLeftTop",
      $svg.querySelector(".croppy-svg-corner-left-top")
    );
    this._storeSetSvgNode(
      "cornerRightTop",
      $svg.querySelector(".croppy-svg-corner-right-top")
    );
    this._storeSetSvgNode(
      "cornerLeftBottom",
      $svg.querySelector(".croppy-svg-corner-left-bottom")
    );
    this._storeSetSvgNode(
      "cornerRightBottom",
      $svg.querySelector(".croppy-svg-corner-right-bottom")
    );

    this._storeSetSvgNode("barTop", $svg.querySelector(".croppy-svg-bar-top"));
    this._storeSetSvgNode(
      "barLeft",
      $svg.querySelector(".croppy-svg-bar-left")
    ),
      this._storeSetSvgNode(
        "barRight",
        $svg.querySelector(".croppy-svg-bar-right")
      ),
      this._storeSetSvgNode(
        "barBottom",
        $svg.querySelector(".croppy-svg-bar-bottom")
      ),
      this.setSizes();
    this._setEventListeners();
  };
