...
const store = {
  ...
  findBlockByGameId(blockId, dataId) {
    const dataIrrelevant = [
      18, 99, 100, 202, 216, 235, 236, 237, 238, 239, 240, 241, 242, 243, 244,
  ...
  setEventListeners() {
    this.convertWorker.onmessage = (e) => {
      ...
      this.mineartCanvas.setImageSizes(
        store.canvasTemp.width,
        store.canvasTemp.height
      );
    ...
    window.addEventListener("load", function (event) {
      if (store.waitForLoaded.arr !== null) {
        ...
        store.mineartCanvas.setImageSizes(
          store.waitForLoaded.width,
          store.waitForLoaded.height
        );
    ...
    document.addEventListener("keydown", (e) => {
      if (
        ...
        document
          .querySelector("section.editor-screen")
      ...
      switch (e.which) {
        ...
        case 69:
          document
            .querySelector('.editor-tools input[value="eyedropper"]')
            .click();
          break;
        case 90:
          if (e.ctrlKey) {
        ...
        case 219:
          if (
            store.editorScreen.currentTool === "pencil" ||
            store.editorScreen.currentTool === "brush"
          ) {
