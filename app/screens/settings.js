...
const store = {
  ...
  init(parent) {
    ...
    if (store.parent.localStorage.getItem("minecraftVersion")) {
      this.$version.value =
        store.parent.localStorage.getItem("minecraftVersion");
    }
  },
  fillTable() {
  ...
  filterTable(name) {
    ...
    store.parent.blocksDefault.forEach((item) => {
      let row = this.$tableBlocks.querySelector(
        `input[data-block-id='${item.id}']`
  ...
  setImageSizesString(w, h, crop) {
    this.$imgSizes.innerHTML = `Image size${
      crop ? " (cropped)" : ""
    }: ${w}x${h}`;
  },
  setEqualsString() {
  ...
  setTableCounter() {
    ...
    const int = this.$tableBlocks.querySelectorAll(
      "tr.visible td input:checked"
    ).length;
  ...
  selectBoxGroup(group) {
    ...
    document
      .getElementById("settings-checkboxes-include")
    ...
    switch (group) {
      ...
      case "custom":
        ...
        document
          .querySelector(".table-blocks-container")
          .classList.remove("hidden");
        document
          .getElementById("settings-checkboxes-include")
  ...
  drawToCanvas() {
    ...
    if (this.$checkCrop.checked) {
      ...
      if (store.parent.uploadedImage.naturalHeight > 500) {
        cropInfo.height =
          (store.parent.uploadedImage.naturalHeight /
            store.$imgPresentation.height) *
          cropInfo.height;
        cropInfo.width =
          (store.parent.uploadedImage.naturalWidth /
            store.$imgPresentation.width) *
          cropInfo.width;
        cropInfo.offsetY =
          (store.parent.uploadedImage.naturalHeight /
            store.$imgPresentation.height) *
      ...
      this.ctxTemp.drawImage(
        ...
        0,
        0,
        store.parent.canvasTemp.width,
        store.parent.canvasTemp.height
      );
      store.parent.mineartCanvas.setOriginalImage({
        src: store.parent.uploadedImage.src,
        x: 0,
  ...
  resetScreen() {
    ...
    document
      .querySelectorAll("#settings-checkboxes-include input")
      .forEach((item) => {
        item.checked = false;
      });
    document.querySelector(
      '#settings-checkboxes-include input[name="include-falling"]'
    ).checked = true;
  ...
  setEventListeners() {
    this.$inputWidth.oninput = (e) => {
      ...
      if (!this.ignoreRatio) {
        this.$inputHeight.value = Math.round(
          this.$inputWidth.value / this.aspectRatio
        );
      }
    ...
    this.$inputHeight.oninput = (e) => {
      e.target.value = e.target.value.replace(/\D/gi, "");
      if (!this.ignoreRatio) {
        this.$inputWidth.value = Math.round(
          this.$inputHeight.value * this.aspectRatio
        );
      }
      this.setEqualsString();
    };
    ...
    this.$submit.onclick = (e) => {
      ...
      const blockGroup = document.querySelector(
        "input[name=block-groups]:checked"
      ).value;

      const parsedVersion = parseInt(this.$version.value);
      if (
        ((18 > parsedVersion && this.$inputHeight.value > 256) ||
          (parsedVersion >= 18 && this.$inputHeight.value > 384)) &&
        this.$ignoreHeightLimit.checked === false
      ) {
        store.parent.errors.triggerError(
          "settings-screen",
          "In-game height limit is 256 blocks for version below 1.18, and 384 (-64 to 320) blocks for version above 1.18.",
          7000
        );
        return;
      }
      ...
      if (this.$inputHeight.value > 1024) {
        store.parent.errors.triggerError(
          "settings-screen",
          "Maximum convert height is 1024.",
          7000
        );
        return;
      }
      ...
      switch (blockGroup) {
        ...
        case "custom":
          ...
          let int = 0;
          this.$tableBlocks
            .querySelectorAll("tr.visible td input")
            .forEach((item) => {
          ...
          if (int >= max - 1) {
            store.parent.errors.triggerError(
              "settings-screen",
              "Please select at least 2 blocks.",
              7000
            );
            return;
          }
      ...
      if (blockGroup === "all" || blockGroup === "survival") {
        if (
          document.querySelector("input[name=include-transparent]").checked ===
          false
        ) {
          store.parent.blocksDefault.forEach((item) => {
        ...
        if (
          document.querySelector("input[name=include-falling]").checked ===
          false
        ) {
          store.parent.blocksDefault.forEach((item) => {
        ...
        if (
          document.querySelector("input[name=include-redstone]").checked ===
          false
        ) {
          store.parent.blocksDefault.forEach((item) => {
        ...
        if (
          document.querySelector("input[name=include-luminance]").checked ===
          false
        ) {
          store.parent.blocksDefault.forEach((item) => {
      ...
      store.parent.convertWorker.postMessage({
        imgData: this.ctxTemp.getImageData(
          0,
          0,
          store.parent.canvasTemp.width,
          store.parent.canvasTemp.height
