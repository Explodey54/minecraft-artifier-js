...
const store = {
  ...
  filterBlockList(str) {
    store.parent.blocksDefault.forEach((item) => {
      ...
      if (item.name.match(regex)) {
        this.$blockList
          .querySelector(`img[data-block-id="${item.id}"]`)
          .classList.remove("hidden");
      } else {
        this.$blockList
          .querySelector(`img[data-block-id="${item.id}"]`)
  ...
  resetScreen() {
    ...
    document.querySelector(
      '.editor-tools input[value="pencil"]'
    ).checked = true;
    ...
    if (store.parent.localStorage.getItem("gridColor")) {
      this.$settingsGridColor.value =
        store.parent.localStorage.getItem("gridColor");
      store.parent.mineartCanvas.setSettingsValue(
        "gridColor",
        this.$settingsGridColor.value
      );
    }
  ...
  undoOnce() {
    ...
    const node = document.querySelector(
      `.info-panels-history-action[data-action-pos="${temp}"]`
    );
    if (temp < -1) {
      return;
    }
  ...
  setEventListeners() {
    ...
    this.$canvas.onclick = (e) => {
      store.parent.mineartCanvas.render();
      const info = store.parent.mineartCanvas.getBlockInfoByMouseXY(
        e.x,
        e.y
      ).info;
      if (!info) {
      ...
      if (
        ...
        store.parent.mineartCanvas.getTool() === "clicker"
      ) {
    ...
    this.$canvas.addEventListener("history", (e) => {
      ...
      const actions = this.$history.querySelectorAll(
        ".info-panels-history-action"
      );
      actions.forEach((item) => {
        item.classList.remove("info-panels-history-action-current");
        if (
          parseInt(item.dataset.actionPos) >= parseInt(this.currentHistoryPos)
        ) {
          item.remove();
          historyCounter--;
        } else {
      ...
      if (historyCounter === this.maxHistory) {
        const lastAction = this.$history.querySelector(
          ".info-panels-history-action"
        );
        lastAction.remove();
      }
      ...
      node.innerHTML =
        e.details.type.charAt(0).toUpperCase() + e.details.type.slice(1);
      node.onclick = () => {
        ...
        this.$firstAction.classList.remove(
          "info-panels-history-action-current"
        );
        const actions = this.$history.querySelectorAll(
          ".info-panels-history-action"
        );
        actions.forEach((item) => {
          item.classList.remove("info-panels-history-action-current");
          if (
            parseInt(item.dataset.actionPos) > parseInt(node.dataset.actionPos)
          ) {
    ...
    this.$saveBtn.onclick = () => {
      ...
      const link = store.parent.mineartCanvas.save(
        this.$settingsWeDirection.value
      );
    ...
    this.$fileInputImage.oninput = (e) => {
      ...
      if (output) {
        ...
        store.parent.errors.triggerError(
          "editor-screen",
          "Wrong file type. Try image file (jpg, png, bmp).",
          5000
        );
        return;
      }
      if (ext.match(/(jpeg|jpg|png|bmp)/i)) {
        ...
        store.parent.startScreen.uploadImage(
          _URL.createObjectURL(e.target.files[0])
        );
      } else {
        store.parent.errors.triggerError(
          "editor-screen",
    ...
    this.$fileInputData.oninput = (e) => {
      ...
      if (output) {
        ...
        store.parent.errors.triggerError(
          "editor-screen",
          "Wrong file type. Try .schematic file.",
          5000
        );
        return;
      }
