...
const store = {
  ...
  convert() {
    ...
    this.commBlockStrings = store.parent.mineartCanvas.convertAsCommandBlock(
      this.$selectFacing.value
    );
    this.mcfunctionBlob = new Blob(
      [
        store.parent.mineartCanvas.convertAsMcfunction(
          this.$selectFacing.value
        ),
      ],
      { type: "text/plain" }
    );
    this.rawCommands = store.parent.mineartCanvas.convertAsRaw(
      this.$selectFacing.value
    );
    ...
    this.counterRaw.setCallback((int) => {
      const slice = this.rawCommands.slice(
        (int - 1) * commandsOnPage,
        commandsOnPage * int
      );
      let output = "";
      slice.forEach((item) => {
      ...
      this.$preRaw.style.counterIncrement =
        "line " + (int - 1) * commandsOnPage;
    ...
    this.counterManual.setMax(
      Math.ceil(this.quantityOfBlocks.length / rowsOnPage)
    );
    this.counterManual.setCallback((int) => {
      $tbody.innerHTML = "";
      const slice = this.quantityOfBlocks.slice(
        (int - 1) * rowsOnPage,
        rowsOnPage * int
      );
      slice.forEach((item) => {
    ...
    this.$stringDelete.innerHTML = store.parent.mineartCanvas
      .getCommandToDelete(this.$selectFacing.value)
      .join("<br>");
    this.$notifyChanged.classList.add("hidden");
  },
  resetScreen() {
    this.$selectVersion.value = 20;
