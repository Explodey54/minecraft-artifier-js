...
const store = {
    ...
    init(parent) {
    	this.parent = parent
    	const nodeList = document.querySelectorAll('*[id^="start-"]').forEach((item) => {
    		let key = '$'
    ...
    uploadImage(src) {
        ...
        store.parent.uploadedImage.onload = () => {
            ...
            store.parent.settingsScreen.$inputWidth.value = store.parent.uploadedImage.width
            store.parent.settingsScreen.$inputHeight.value = store.parent.uploadedImage.height
            store.parent.settingsScreen.aspectRatio = store.parent.uploadedImage.width / store.parent.uploadedImage.height
            store.parent.settingsScreen.setImageSizesString(store.parent.uploadedImage.naturalWidth, store.parent.uploadedImage.naturalHeight)
    ...
    setEventListeners() {
        ...
        this.$dropzone.ondrop = (e) => {
            ...
            if (e.dataTransfer.files.length !== 1) {
                store.parent.errors.triggerError('start-screen', 'Please, choose only one file.', 5000)
                return
            }
